...
def k_means(histograms, k=5, num_iterations=100):
    # Initialize centroids randomly from the dataset
    centroids = histograms[np.random.choice(histograms.shape[0], k, replace=False)]

    for _ in range(num_iterations):
        # Assign clusters
        distances = np.sqrt(((histograms - centroids[:, np.newaxis])**2).sum(axis=2))
        closest_cluster = np.argmin(distances, axis=0)

        # Update centroids
        for i in range(k):
            centroids[i] = histograms[closest_cluster == i].mean(axis=0)
...
def main():
    ...
    cluster_labels = k_means(image_histograms)
    ...
    plt.savefig(f"confusion_matrix/self-implemented-k-means-confusion-matrix.png")
    plt.close()
